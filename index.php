...
function searchEqualsInArrays() {
    ...
    for ($i = 0; $i < $argsAmount; $i++) {
        for ($k = 0; $k < sizeof($args[$i]); $k++) {
            for ($t = 0; $t < $argsAmount; $t++) {
                if ($t == $i) { // ищем совпадения только в других массивах
                    continue;
                }   
                if (in_array($args[$i][$k], $args[$t]) && !in_array($args[$i][$k], $result)) {
                    $result[] = $args[$i][$k];
                }
            }
        }
